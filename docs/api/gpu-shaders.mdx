...
Web `2.42.0` adds the experimental JavaScript option
...
## Enabling GPU Canvas on Web 2.42.0 {#enabling-gpu-canvas-on-web-2420}
...
GPU Canvas content rendered. React Canvas `4.34.0` forwards the same option to
Canvas `2.42.0`. Do not explicitly combine GPU Canvas with the shared offscreen
renderer because the two rendering modes are incompatible.

This option does not change the C++ `runtime-v0.1.344` Luau declarations or
