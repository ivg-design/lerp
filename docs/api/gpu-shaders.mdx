...
:::warning Web host opt-in
Web `2.42.1` adds the experimental JavaScript option
`enableGPUCanvas?: boolean`. It defaults to `false`; GPU Canvas content is not
enabled merely because a file contains a shader or the Luau analyzer recognizes
GPU types. This is host configuration, not a Luau input or global.
:::
...
## Enabling GPU Canvas on Web 2.42.1 {#enabling-gpu-canvas-on-web-2420}

Set `enableGPUCanvas: true` when the Web host imports a file from `src` or
`buffer`:
...
The option makes the Web host import GPU Canvas content through deferred
rendering. It is experimental, defaults to `false`, and is fixed at file
import. If you create a reusable `RiveFile`, choose the mode there:
...
const rive = new Rive({ riveFile, canvas })
```

When a `RiveFile` is supplied, that file's setting wins; a `Rive` instance
cannot switch the imported resources between immediate and deferred modes.
Create a new `RiveFile` to change the mode. A deferred session records for one
canvas, so a second `Rive` instance sharing that file re-imports it into its own
session. If the selected runtime build cannot create a deferred session, it
warns and imports in immediate mode; setting the flag alone is not proof that
GPU Canvas content rendered. The LERP landing's bundled React Canvas `4.34.0`
(resolving Canvas `2.42.0`, one wrapper patch behind the `2.42.1` direct
target above; see [Runtime Compatibility
Baseline](/rive/runtime-compatibility#landing-integration-lag)) forwards the
same option to its resolved Canvas build. Do not explicitly combine GPU Canvas
with the shared offscreen renderer because the two rendering modes are
incompatible.

This option does not change the C++ `runtime-v0.1.384` Luau declarations or
the Rive Luau LSP. It also does not establish editor packaging, export, RAV, or
third-party host support. Test an exported GPU Canvas file in the exact host
before relying on it. The LERP landing keeps the option off because its
checked-in files do not require GPU Canvas.
