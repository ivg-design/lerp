...
`NodeReadData.paint`, `node:asPath()`, and `node:asPaint()` are validated as live in npm runtime baseline `2.37.6` with upstream source snapshot `runtime-v0.1.64` (`b25a322`, audited May 12, 2026).
