...
sidebar_position: 3
title: Scene
description: "Rive Luau scene API reference for artboards, nodes, and animations: properties, rendering, advancement, and independent instances."
keywords:
  - rive
  - luau
  - scripting
...
| `data` | typed | read-only | Associated <Term>ViewModel</Term> data |
...
Creates an independent copy, optionally bound to a ViewModel.

```lua
artboard:instance(viewModel?: ViewModel): Artboard
```
...
Gets a node's data by name.

```lua
artboard:node(name: string): NodeData?
```

#### `artboard:addToPath(path, transform?)`

Adds the artboard's geometry to a <Term>Path</Term>, optionally transformed.
...
The hosting Node is a scene-hierarchy relationship, not an extra lifecycle parameter. Nesting a script node under a group or shape affects transform inheritance, opacity, and draw order. A Node script still receives the normal Node protocol values (`context`, `renderer`, `seconds`, pointer events) and does not automatically receive the parent shape's `NodeData` or `PathData`.
...
Called when an <Term>Input</Term> value changes.
...
**See Also:** [PointerEvent](./events#pointerevent), [<Term>Renderer</Term>](./drawing#renderer), [NodeData](./hierarchy#nodedata)
...
- Continue to [Data & Input](/api/data-input)
