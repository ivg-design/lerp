...
exact `runtime-v0.1.384` target. Current public docs and the current editor
...
The exact C++ Luau Context in `runtime-v0.1.384` has no matching `log` atom or
dispatch case. Use `print(...)` in portable Web `2.42.1` target scripts, and
...
At the accepted released compatibility boundary (`@rive-app/canvas@2.42.1`, C++
`runtime-v0.1.384`), `viewModel.name` remains a documentation/source distinction
