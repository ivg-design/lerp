...
- Keep long-running integer counters 32-bit safe (clamp/reset as needed)
...
Shader workflows add GPU resources to the same lifecycle discipline:
...
- Use `loadOp = "clear"` unless previous attachment contents are required.
- Use `storeOp = "discard"` for transient MSAA and depth attachments after resolve.

For shader-specific API details, see [GPU Shaders](/api/gpu-shaders).
