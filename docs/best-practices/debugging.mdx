...
**Goal**: Use print statements to understand when lifecycle functions are called.
...
function init(self: LifecycleTracer): boolean
    print(`[{self.name.value}] init called`)
    self.frameCount = 0
    self.path = Path.new()
...
function update(self: LifecycleTracer, context: Context)
    print(`[{self.name.value}] update called - input changed`)
end

function advance(self: LifecycleTracer, seconds: number): boolean
    self.frameCount += 1

    -- Print only first 3 frames to avoid spam
    if self.frameCount <= 3 then
        print(`[{self.name.value}] advance #{self.frameCount}, dt={seconds:.4f}`)
    end
...
function draw(self: LifecycleTracer, renderer: Renderer)
    -- Avoid printing in draw - it runs every frame!
    self.path:reset()
...
Change the `name` input in the inspector to see:
```
[Tracer] update called - input changed
```
...
**Goal**: Track how values change over time.
...
local function logState(label: string, self: StateDebugger)
    print(`[{label}] target={self.targetX.value}, current={self.currentX:.2f}, vel={self.velocity:.2f}`)
end
...
function advance(self: StateDebugger, seconds: number): boolean
    ...
    -- Spring physics
    local displacement = self.targetX.value - self.currentX
...
return function(): Node<StateDebugger>
    return {
        ...
        velocity = 0,
        path = Path.new(),
        paint = Paint.with({ style = "fill", color = Color.rgb(80, 180, 120) }),
    }
end
```
...
**Goal**: Find and fix the bugs in this script.
...
function init(self: BuggyCircle): boolean
    -- Bug 1: Missing initialization
    self.path:reset()
    self.path:moveTo(Vector.xy(-self.radius.value, 0))
...
function advance(self: BuggyCircle, seconds: number): boolean
    -- Bug 3: Accessing wrong field
    local r = self.rad.value
    print("Radius:", r)
    return true
end
...
export type FixedCircle = {
    radius: Input<number>,
    path: Path,
    paint: Paint,
}

function init(self: FixedCircle): boolean
    -- Fix 1: Initialize path before using
    self.path = Path.new()
    self.path:reset()
    ...
    -- Fix 2: Correct property name (paint, not colour)
...
function advance(self: FixedCircle, seconds: number): boolean
    -- Fix 3: Correct field name (radius, not rad)
    local r = self.radius.value
...
function draw(self: FixedCircle, renderer: Renderer)
    renderer:drawPath(self.path, self.paint)
end

return function(): Node<FixedCircle>
    return {
        init = init,
        advance = advance,
        draw = draw,
