...
Web `2.42.1` backed by `runtime-v0.1.384` calls
...
Web `2.42.1` includes layout and animation correctness fixes for integer grid
...
If the callback is omitted, `runtime-v0.1.384` performs no script resize work.
...
<Quiz
  ...
  explanation="resize() receives the granted size and display scale when it is implemented. At runtime-v0.1.384 it is optional, and a missing callback is a no-op."
/>
