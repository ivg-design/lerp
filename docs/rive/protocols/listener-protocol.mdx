...
This page is aligned to npm runtime line `2.37.6` with upstream source snapshot `runtime-v0.1.64` (`b25a322`, audited May 12, 2026). ListenerAction event payloads (including gamepad payload support) are documented in [Events → ListenerContext](/api/events#listenercontext).
:::
...
| State machine listener action payloads | N/A | Framework/event object | `performAction(self, listenerContext)` in ListenerAction scripts |
...
**Luau (Rive)**: Three event mechanisms:
...
3. **ListenerAction payloads** - `performAction(self, listenerContext)` receives tagged event context from state machine listeners
...
<Quiz
  question="In a ListenerAction script, which pattern safely reads payload data?"
  id="other-q2"
  options={[
    { text: "Call asPointerEvent() directly and assume it never returns nil." },
    { text: "Branch with listenerContext:is...(), then call the matching as...() accessor.", correct: true },
    { text: "Use update(self) to access listener payloads." },
    { text: "Read pointer fields from context:viewModel()." },
  ]}
  explanation="ListenerContext is a tagged union. Use is... guards first, then as... accessors to avoid invalid field access."
/>
...
3. **ListenerAction payload guards** - Use `listenerContext:is...()` + `as...()` for state machine listener actions
4. **EventBus pattern** - Util scripts enable decoupled component communication
5. **State machine inputs (legacy)** - Useful for integrating older files; prefer ViewModels for new data flows
6. **Drag behavior** - Requires `pointerDown`, `pointerMove`, and `pointerUp` together
