---
sidebar_position: 6
title: Path Effect Script Protocol
description: Real-time manipulation of path geometry for stroke and fill effects
keywords:
  - rive
  - luau
  - scripting
  - rive
  - path
  - effect
  - script
  - protocol
---

import Quiz from '@site/src/components/Quiz';
import ExerciseValidator from '@site/src/components/ExerciseValidator';
import Term from '@site/src/components/Term';

# Path Effect Script Protocol

## Learning Objectives

- Understand when to use Path Effect Scripts
- Process PathData and its command types
- Implement the `update()` function to transform paths
- Create animated path effects with `advance()`

---

## AE/JS Syntax Comparison

| Concept | JavaScript (Canvas/SVG) | Luau (Rive Path Effects) |
|---------|-------------------------|--------------------------|
| Path creation | `new Path2D()` | `Path.new()` |
| Move to | `path.moveTo(x, y)` | `path:moveTo(Vector.xy(x, y))` |
| Line to | `path.lineTo(x, y)` | `path:lineTo(Vector.xy(x, y))` |
| Cubic curve | `path.bezierCurveTo(...)` | `path:cubicTo(cp1, cp2, end)` |
| Close path | `path.closePath()` | `path:close()` |

:::tip Key Insight
Path Effect Scripts receive the **original path** and return a **transformed path**. Unlike SVG filters that apply post-render effects, Path Effects modify the actual geometry before rendering.
:::

---

## Overview

Path Effect Scripts enable **real-time manipulation of path geometry**. They give you programmatic control over the shape and structure of paths for effects like warping, distortion, animation, and procedural modifications.

**When to use Path Effect Scripts:**
- Custom stroke and fill effects (waves, distortions, warps)
- Procedural path generation
- Animated path modifications
- Geometric transformations on existing paths

---

## The Path Effect Script Template

```lua
export type MyPathEffect = {
    context: Context,
    time: number,
    amplitude: Input<number>,
    frequency: Input<number>,
}

function init(self: MyPathEffect, context: Context): boolean
    self.context = context
    self.time = 0
    return true
end

-- Called each frame for animated effects
function advance(self: MyPathEffect, seconds: number): boolean
    self.time += seconds
    return true  -- Return true to keep receiving advance calls
end

-- Core transformation: receives original path and host node, returns modified path
function update(self: MyPathEffect, inPath: PathData, node: NodeReadData): PathData
    local path = Path.new()

    for _, cmd in ipairs(inPath) do
        if cmd.type == "moveTo" then
            path:moveTo(cmd[1])

        elseif cmd.type == "lineTo" then
            path:lineTo(cmd[1])

        elseif cmd.type == "quadTo" then
            path:quadTo(cmd[1], cmd[2])

        elseif cmd.type == "cubicTo" then
            path:cubicTo(cmd[1], cmd[2], cmd[3])

        elseif cmd.type == "close" then
            path:close()
        end
    end

    return path
end

return function(): PathEffect<MyPathEffect>
    return {
        init = init,
        advance = advance,
        update = update,
        context = late(),
        time = 0,
        amplitude = late(),
        frequency = late(),
    }
end
```

---

## Type Signature Reference

### Path Effect Protocol Functions

| Function | Signature | Required | Description |
|----------|-----------|----------|-------------|
| `init` | `(self: T, context: Context): boolean` | No | Initialize effect state |
| `update` | `(self: T, inPath: PathData, node: NodeReadData): PathData` | **Yes** | Transform path geometry |
| `advance` | `(self: T, seconds: number): boolean` | No | Per-frame animation updates |

### Factory Return Type

```lua
return function(): PathEffect<YourEffectType>
    return {
        init = init,      -- Optional
        update = update,  -- Required
        advance = advance, -- Optional (for animated effects)
        context = late(),
        time = 0,
        -- ... your inputs
    }
end
```

### PathData Command Types

| Command Type | Points | Description |
|--------------|--------|-------------|
| `"moveTo"` | 1 Vector (`cmd[1]`) | Move to point |
| `"lineTo"` | 1 Vector (`cmd[1]`) | Line to point |
| `"quadTo"` | 2 Vectors (`cmd[1]`=control, `cmd[2]`=end) | Quadratic curve |
| `"cubicTo"` | 3 Vectors (`cmd[1]`=cp1, `cmd[2]`=cp2, `cmd[3]`=end) | Cubic curve |
| `"close"` | 0 | Close path |

> **Note:** Each point is a `Vector` object. Access coordinates via `cmd[1].x` and `cmd[1].y`.
>
> **Editor note:** In currently validated editor scripts, compare `cmd.type` against string names (`"moveTo"`, `"lineTo"`, etc.). Do not assume a global `CommandType` table exists at runtime.

---

## Key Functions

### `update(self, inPath: PathData, node: NodeReadData): PathData` — **REQUIRED**

The core transformation function. Receives the original path data, returns the modified path.

```lua
function update(self: MyPathEffect, inPath: PathData, node: NodeReadData): PathData
    local path = Path.new()

    for _, cmd in ipairs(inPath) do
        -- Process each command and build new path
    end

    return path
end
```

---

### `advance(self, seconds: number): boolean` — Optional

Called each frame for time-based animations. Use this to update internal state like time counters.

```lua
function advance(self: MyPathEffect, seconds: number): boolean
    self.time += seconds
    return true  -- Return true to keep receiving callbacks
end
```

**Return value:**
- `true` — Continue receiving advance callbacks
- `false` — Stop receiving callbacks (static effect)

---

### `init(self, context: Context): boolean` — Optional

One-time initialization when the effect is first applied.

```lua
function init(self: MyPathEffect, context: Context): boolean
    self.context = context
    self.time = 0
    return true
end
```

---

## PathData API

PathData is an indexed collection of PathCommand objects representing the original path geometry.

### Iterating Commands

```lua
-- Get total number of commands
local count = #inPath

-- Iterate through all commands
for _, cmd in ipairs(inPath) do
    local cmdType = cmd.type     -- command name string (for example "lineTo")
    local pointCount = #cmd      -- Number of points (Vectors)

    -- Access points by index
    local point = cmd[1]
    if point then
        print(point.x, point.y)
    end
end
```

---

## PathCommand Type Values (`cmd.type`)

| Type | Points | Values | Description |
|------|--------|--------|-------------|
| `"moveTo"` | 1 | `cmd[1]` = Vector | Move cursor to position |
| `"lineTo"` | 1 | `cmd[1]` = Vector | Draw straight line |
| `"quadTo"` | 2 | `cmd[1]`=control, `cmd[2]`=end | Quadratic bezier curve |
| `"cubicTo"` | 3 | `cmd[1]`=cp1, `cmd[2]`=cp2, `cmd[3]`=end | Cubic bezier curve |
| `"close"` | 0 | *(none)* | Close path to start |

---

## How to Apply Path Effects

1. Create a Path Effect Script
2. In Editor: Select a stroke or fill → Effects Tab
3. Add **Script Effect** → Choose your script
4. Configure any custom inputs

---

## Practice Exercises

### Exercise 1: Pass-Through Effect ⭐ {#exercise-1-passthrough-effect}

### Premise

Before modifying paths, you need to understand how to copy them unchanged. This establishes the foundation for all path transformations.

:::info Goal
By the end of this exercise, you will create a path effect that copies the <Term>Input</Term> path without modification.
:::

### Use Case

You're building a debugging tool to verify path data is being processed correctly before adding transformations.

---

### Setup

#### In Rive Editor:

1. **Create the script:**
   - Assets panel → `+` → Script → Path Effect Script
   - Name it `Exercise1_PassThrough`

2. **Apply to a shape:**
   - Select any path/shape with a stroke or fill
   - Effects tab → Add Script Effect → Choose your script

---

### Starter Code

```lua
export type Exercise1 = {
    context: Context,
}

function init(self: Exercise1, context: Context): boolean
    self.context = context
    return true
end

function update(self: Exercise1, inPath: PathData, node: NodeReadData): PathData
    local path = Path.new()
    local cmdCount = 0

    for _, cmd in ipairs(inPath) do
        cmdCount += 1

        -- TODO: Handle each command type
        if cmd.type == "moveTo" then
            path:moveTo(cmd[1])

        elseif cmd.type == "lineTo" then
            -- TODO: Copy lineTo command
            path:lineTo(cmd[1])

        elseif cmd.type == "quadTo" then
            -- TODO: Copy quadTo command (control + end)
            path:quadTo(cmd[1], cmd[2])

        elseif cmd.type == "cubicTo" then
            -- TODO: Copy cubicTo with all 3 vectors
            -- Hint: cmd[1]=controlOut, cmd[2]=controlIn, cmd[3]=end
            path:cubicTo(cmd[1], cmd[2], cmd[3])

        elseif cmd.type == "close" then
            path:close()
        end
    end

    print(`ANSWER: commands={cmdCount}`)
    return path
end

return function(): PathEffect<Exercise1>
    return {
        init = init,
        update = update,
        context = late(),
    }
end
```

---

### Assignment

1. Complete the `quadTo` and `cubicTo` handlers with the correct vector parameters
2. Apply to a simple shape (circle or rectangle)
3. Count should be non-zero if path has commands

---

### Expected Output

Your console output should display the number of path commands processed.

---

### Verify Your Answer

<ExerciseValidator
    exerciseId="rive-pathfx-1"
    expectedAnswer="ANSWER: commands="
    matchType="startsWith"
/>

---

### Checklist

- [ ] moveTo copies `cmd[1]`
- [ ] lineTo copies `cmd[1]`
- [ ] quadTo copies `cmd[1]` and `cmd[2]`
- [ ] cubicTo copies `cmd[1]`, `cmd[2]`, and `cmd[3]`
- [ ] close is handled
- [ ] Path renders identically to original

---

### Exercise 2: Vertical Offset Effect ⭐⭐ {#exercise-2-vertical-offset-effect}

### Premise

Now that you can copy paths, let's modify them. Adding an offset to coordinates is the simplest transformation.

:::info Goal
By the end of this exercise, you will create a path effect that shifts all points vertically.
:::

### Use Case

You want to create a shadow effect by duplicating and offsetting a path.

---

### Setup

#### In Rive Editor:

1. **Create the script:**
   - Assets panel → `+` → Script → Path Effect Script
   - Name it `Exercise2_VerticalOffset`

---

### Starter Code

```lua
export type Exercise2 = {
    context: Context,
    offsetY: Input<number>,
}

function update(self: Exercise2, inPath: PathData, node: NodeReadData): PathData
    local path = Path.new()
    local offset = self.offsetY or 20

    for _, cmd in ipairs(inPath) do
        if cmd.type == "moveTo" then
            -- TODO: Add offset to Y coordinate
            -- Original: cmd[1] is a Vector
            local p = cmd[1]
            local x = p.x
            local y = p.y + offset

            path:moveTo(Vector.xy(x, y))

        elseif cmd.type == "lineTo" then
            -- TODO: Add offset to Y coordinate
            local p = cmd[1]
            path:lineTo(Vector.xy(p.x, p.y + offset))

        elseif cmd.type == "quadTo" then
            -- TODO: Add offset to control and end points
            local c = cmd[1]
            local e = cmd[2]
            path:quadTo(
                Vector.xy(c.x, c.y + offset),
                Vector.xy(e.x, e.y + offset)
            )

        elseif cmd.type == "cubicTo" then
            -- TODO: Add offset to all 3 points
            local c1 = cmd[1]
            local c2 = cmd[2]
            local e = cmd[3]
            path:cubicTo(
                Vector.xy(c1.x, c1.y + offset),
                Vector.xy(c2.x, c2.y + offset),
                Vector.xy(e.x, e.y + offset)
            )

        elseif cmd.type == "close" then
            path:close()
        end
    end

    print(`ANSWER: offset={offset}`)
    return path
end

return function(): PathEffect<Exercise2>
    return {
        update = update,
        context = late(),
        offsetY = 20,
    }
end
```

---

### Assignment

1. Verify all Y coordinates have the offset added
2. Test with offsetY = 20

---

### Expected Output

Your console output should display the offset value being applied.

---

### Verify Your Answer

<ExerciseValidator
    exerciseId="rive-pathfx-2"
    expectedAnswer="ANSWER: offset=20"
/>

---

### Checklist

- [ ] moveTo Y coordinate offset
- [ ] lineTo Y coordinate offset
- [ ] quadTo control and end Y coordinates offset
- [ ] cubicTo all 3 points Y coordinates offset
- [ ] Path visibly shifts downward

---

### Exercise 3: Wave Distortion ⭐⭐⭐ {#exercise-3-wave-distortion}

### Premise

Animated path effects use `advance()` to track time and create dynamic transformations. Wave distortion is a classic effect.

:::info Goal
By the end of this exercise, you will create an animated wave effect that distorts path geometry.
:::

### Use Case

You're creating a water or wind effect where shapes appear to ripple.

---

### Setup

#### In Rive Editor:

1. **Create the script:**
   - Assets panel → `+` → Script → Path Effect Script
   - Name it `Exercise3_WaveEffect`

2. **Press Play** to see animation

---

### Starter Code

```lua
export type Exercise3 = {
    context: Context,
    time: number,
    amplitude: Input<number>,
    frequency: Input<number>,
}

function init(self: Exercise3, context: Context): boolean
    self.context = context
    self.time = 0
    return true
end

function advance(self: Exercise3, seconds: number): boolean
    -- TODO: Increment time by seconds
    self.time += seconds
    return true
end

-- Helper: Calculate wave offset for a Y position
local function waveOffset(y: number, time: number, amp: number, freq: number): number
    -- TODO: Return sine wave offset
    -- Formula: amp * sin(y * freq + time * 3)
    return amp * math.sin(y * freq + time * 3)
end

function update(self: Exercise3, inPath: PathData, node: NodeReadData): PathData
    local path = Path.new()
    local amp = self.amplitude or 5
    local freq = self.frequency or 0.1
    local t = self.time

    for _, cmd in ipairs(inPath) do
        if cmd.type == "moveTo" then
            local p = cmd[1]
            local offset = waveOffset(p.y, t, amp, freq)
            path:moveTo(Vector.xy(p.x + offset, p.y))

        elseif cmd.type == "lineTo" then
            local p = cmd[1]
            local offset = waveOffset(p.y, t, amp, freq)
            path:lineTo(Vector.xy(p.x + offset, p.y))

        elseif cmd.type == "quadTo" then
            local c = cmd[1]
            local e = cmd[2]
            local oc = waveOffset(c.y, t, amp, freq)
            local oe = waveOffset(e.y, t, amp, freq)
            path:quadTo(
                Vector.xy(c.x + oc, c.y),
                Vector.xy(e.x + oe, e.y)
            )

        elseif cmd.type == "cubicTo" then
            local c1 = cmd[1]
            local c2 = cmd[2]
            local e = cmd[3]
            local o1 = waveOffset(c1.y, t, amp, freq)
            local o2 = waveOffset(c2.y, t, amp, freq)
            local o3 = waveOffset(e.y, t, amp, freq)

            path:cubicTo(
                Vector.xy(c1.x + o1, c1.y),
                Vector.xy(c2.x + o2, c2.y),
                Vector.xy(e.x + o3, e.y)
            )

        elseif cmd.type == "close" then
            path:close()
        end
    end

    print(`ANSWER: time={math.floor(t * 10) / 10},amp={amp}`)
    return path
end

return function(): PathEffect<Exercise3>
    return {
        init = init,
        advance = advance,
        update = update,
        context = late(),
        time = 0,
        amplitude = 5,
        frequency = 0.1,
    }
end
```

---

### Assignment

1. Verify `advance()` increments time
2. Verify wave offset is applied to X coordinates based on Y position
3. Watch the shape animate when playing

---

### Expected Output

Your console output should show time incrementing and amplitude.

---

### Verify Your Answer

<ExerciseValidator
    exerciseId="rive-pathfx-3"
    expectedAnswer="ANSWER: time="
    matchType="startsWith"
/>

---

### Checklist

- [ ] `advance()` increments self.time
- [ ] Wave offset varies by Y position
- [ ] Time component creates animation
- [ ] Shape visibly wobbles when playing

---

## Knowledge Check

<Quiz
  question="What does the update() function receive and return?"
  id="pathfx-q1"
  options={[
    { text: "Receives <Term>Renderer</Term>, returns nothing" },
    { text: "Receives PathData + NodeReadData, returns PathData", correct: true },
    { text: "Receives Path, returns Path" },
    { text: "Receives Vector, returns Vector" },
  ]}
  explanation="update() receives the original PathData and host NodeReadData, then returns PathData (the transformed path)."
/>

<Quiz
  question="How many points does a cubicTo command have?"
  id="pathfx-q2"
  options={[
    { text: "1 point" },
    { text: "2 points" },
    { text: "3 points", correct: true },
    { text: "6 points" },
  ]}
  explanation="cubicTo stores 3 Vector points: controlOut, controlIn, and end."
/>

<Quiz
  question="What should advance() return to keep receiving frame callbacks?"
  id="pathfx-q3"
  options={[
    { text: "Return nothing" },
    { text: "Return false" },
    { text: "Return true", correct: true },
    { text: "Return the time value" },
  ]}
  explanation="Return true from advance() to keep receiving callbacks each frame. Return false to stop (for static effects)."
/>

<Quiz
  question="Which function is REQUIRED in a Path Effect Script?"
  id="pathfx-q4"
  options={[
    { text: "init()" },
    { text: "advance()" },
    { text: "update()", correct: true },
    { text: "draw()" },
  ]}
  explanation="update() is the only required function - it's the core transformation that converts input PathData to output PathData."
/>

---

## Common Mistakes

:::danger Avoid These Errors

1. **Forgetting to handle all command types**
   ```lua
   -- WRONG: Only handling moveTo and lineTo
   for _, cmd in ipairs(inPath) do
       if cmd.type == "moveTo" then ...
       elseif cmd.type == "lineTo" then ...
       -- Missing cubicTo, quadTo, close!
   end

   -- CORRECT: Handle all types
   for _, cmd in ipairs(inPath) do
       if cmd.type == "moveTo" then ...
       elseif cmd.type == "lineTo" then ...
       elseif cmd.type == "cubicTo" then ...
       elseif cmd.type == "quadTo" then ...
       elseif cmd.type == "close" then ...
       end
   end
   ```

2. **Treating PathCommand points like raw number pairs**
   ```lua
   -- WRONG: cmd[1] is already a Vector
   local x = cmd[1]
   local y = cmd[2]
   path:moveTo(Vector.xy(x, y))

   -- CORRECT: Use Vector points directly
   local p = cmd[1]
   path:moveTo(Vector.xy(p.x, p.y))

   -- Also valid when not modifying coordinates:
   path:moveTo(cmd[1])
   ```

3. **Wrong argument mapping for cubicTo**
   ```lua
   -- WRONG: Mixing components across points
   path:cubicTo(
       Vector.xy(cmd[1].x, cmd[2].y),
       Vector.xy(cmd[2].x, cmd[3].y),
       cmd[3]
   )

   -- CORRECT: cmd[1], cmd[2], cmd[3] are the 3 points
   path:cubicTo(
       cmd[1],  -- controlOut
       cmd[2],  -- controlIn
       cmd[3]   -- end
   )
   ```

4. **Not returning true from advance()**
   ```lua
   -- WRONG: Animation stops after first frame
   function advance(self, seconds)
       self.time += seconds
       -- No return! Defaults to nil/false
   end

   -- CORRECT: Keep receiving callbacks
   function advance(self, seconds)
       self.time += seconds
       return true
   end
   ```

5. **Returning a non-path value from update()**
   ```lua
   -- WRONG: Returning something that's not PathData
   function update(self, inPath)
       return 0
   end

   -- CORRECT: Return PathData (new or pass-through)
   function update(self, inPath)
       local path = Path.new()
       -- ... build path
       return path
   end
   ```

:::

---

## Self-Assessment Checklist

- [ ] I understand that update() transforms PathData → PathData
- [ ] I can iterate through PathData commands with ipairs
- [ ] I know the point layout for each command type (`cmd[i]` are Vectors)
- [ ] I can create animated effects using advance() and time
- [ ] I understand that advance() should return true for animations

---

## Summary

Path Effect Scripts are the most advanced script type in Rive. They give you complete control over path geometry, enabling:

- **Static effects**: Offsets, scales, custom dash patterns
- **Animated effects**: Waves, wobbles, procedural animation
- **Generative effects**: Procedural path creation

The key is understanding the PathData structure and building new paths command by command.

## Next Steps

- Continue to [Util Protocol](/rive/protocols/util-protocol)
- Need a refresher? Review [Quick Reference](/quick-reference)
