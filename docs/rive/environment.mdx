...
- ❌ No Roblox runtime APIs (`game`, `workspace`, `Instance`, services)
...
## Authoring Outside Rive

The Rive editor remains the final runtime environment for scripts, but you do not have to write every line inside the editor. If you prefer VS Code, Cursor, Windsurf, or another VS Code-compatible editor, install the **[Rive Luau VS Code extension](https://marketplace.visualstudio.com/items?itemName=IVGDesign.rive-luau)**.

The extension is backed by the **[Rive Luau LSP](https://github.com/ivg-design/rive-luau-lsp)**, a language server built for Rive's Luau scripting environment. Use it for Rive-parity linting, diagnostics, autocomplete, hover documentation, and IntelliSense while editing `.luau` files outside Rive.

This is especially useful for shader-era scripts because typed GPU descriptors, `.luau` syntax, `GPUCanvas`, `GPUPipeline`, `GPUBuffer`, `UBOEntry`, `TextureEntry`, and `SamplerEntry` benefit from accurate checking before you paste or sync the script back into Rive.

:::tip External editor workflow
...
Rive supports multiple script types (Node, Layout, Converter, Path Effect, ListenerAction, TransitionCondition, Util, Test). This page focuses on the two foundational patterns you'll use first:
...
function init(self: ConsoleExplorer, context: Context): boolean
    print("init() called")
    self.frameCount = 0
    return true
end
...
function init(self: VectorPractice, context: Context): boolean
...
function init(self: Player, context: Context): boolean
...
function init(self: SandboxDemo, context: Context): boolean
...
:::note Pure Luau in a Rive Runtime
Rive scripting uses Luau syntax and standard Luau libraries, but not Roblox runtime globals. Do not use APIs such as `game`, `workspace`, `Instance`, or Roblox services.
:::

### Numeric Safety (32-bit)

For runtime compatibility, keep integer-style values 32-bit safe unless you explicitly know your target can handle wider ranges.

- Signed 32-bit range: `-2147483648` to `2147483647`
- Good candidates to clamp/reset: frame counters, ids, accumulators, bitwise values
- If values can grow forever, wrap or clamp them predictably
...
5. Save the script to recompile it before testing

### Adding Node Scripts to the Artboard

1. Right-click the artboard and choose your Node script from the script menu
2. Position the script node on the artboard
3. Optionally nest it under a group or shape when you want normal hierarchy behavior such as inherited transform or draw order
4. Configure inputs on the script node in the Inspector

Nesting does not pass the parent node's `NodeData` or `PathData` into Node script callbacks. If you need existing shape path data, use a Path Effect script. If you need a specific scene node, pass it explicitly through an input or retrieve it by name from an artboard reference when that pattern is available.
...
- Rive scripts use pure Luau + Rive APIs (not Roblox runtime APIs)
- Keep integer-style runtime values 32-bit safe for portability
