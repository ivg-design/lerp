...
<Quiz
  question="Why is --!strict recommended at the top of Rive scripts?"
  id="env-q3"
  options={[
    { text: "It makes the script run faster" },
    { text: "It enables type checking which catches errors early", correct: true },
    { text: "It allows network access" },
    { text: "Scripts won't run without it" },
  ]}
  explanation="--!strict enables Luau's type checking, which catches errors before runtime. While scripts can run without it, using strict mode is strongly recommended for catching bugs early."
/>
