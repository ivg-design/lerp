---
sidebar_position: 2
title: Rive Script Types — Choose a Luau Protocol
description: Compare Rive Node, Layout, Converter, Path Effect and state machine scripts. Choose the right Luau protocol and follow its lesson and example.
keywords:
  - rive
  - luau
  - scripting
...
This overview is aligned to direct Web `2.42.1` -> C++
`runtime-v0.1.384`. Current editor-only and later-source surfaces remain explicitly
...
Choose the protocol by what the script needs to control. Drawing new graphics and changing an existing path use different callbacks, even if the results look similar.

| What you want to build | Start here | Why this fits |
|---|---|---|
| Draw procedural shapes or update animation logic each frame | [Node script](./protocols/node-protocol) | Owns the script's drawing and frame-update behavior. |
| Warp an existing stroke or fill | [Path Effect script](./protocols/path-effect-protocol) | Receives the host path geometry to modify. |
| Format a number or transform a bound value | [Converter script](./protocols/converter-protocol) | Transforms values in a data binding. |
| Arrange and size layout children | [Layout script](./protocols/layout-protocol) | Extends Node behavior with layout measurement and resize callbacks. |
| Run an action from a state machine listener | [ListenerAction script](./protocols/listener-action-protocol) | Runs when the configured listener triggers it. |
| Reuse the same math in several scripts | [Util script](./protocols/util-protocol) | Exports shared functions for other scripts to require. |

For advanced custom easing, the [ScriptedInterpolator lesson](./protocols/scripted-interpolator-protocol) works through factor remapping and value blending. Its runtime compatibility is documented separately from editor availability.

## The 8 Script Types

| Script Type | Primary Purpose | Placement / Binding Target |
|-------------|-----------------|-----------|
| **[Node Script](#node-script)** | Animation logic, custom rendering | Scene script node |
...
| **[Path Effect Script](#path-effect-script)** | Modify path geometry | Strokes & Fills |
...
At the Web `2.42.1` target, Rive dispatches
