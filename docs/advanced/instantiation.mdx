...
<Exercise
  id="advanced-instantiation-1"
  title="Exercise 1: Spawn Multiple Instances"
  difficulty={3}
  hint="Call self.star:instance() to create artboard instances. Store them in a table with position data. Call advance() and draw() on each instance."
  expectedAnswer="spawned"
  explanation="Dynamic instantiation creates runtime copies of component artboards. You must call advance() and draw() on each instance every frame."
  task="Complete the spawn function to create instances, and the advance/draw functions to update and render them."
  expectedOutput={`Spawning 5 stars...
5 stars spawned!
ANSWER: spawned`}
  code={`--!strict
...
>
**Premise:** You're building a system that spawns multiple copies of a component artboard and animates them independently.

**Use Case:** Particle systems, enemy spawners, collectible items, and any scenario where you need multiple animated instances of the same component.
</Exercise>
