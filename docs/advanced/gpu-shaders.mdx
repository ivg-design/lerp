...
:::warning Web 2.42.0 activation boundary
Web GPU Canvas playback is experimental and default-off. A JavaScript host
must import the file with `enableGPUCanvas: true`; this setting is fixed at
`RiveFile` import and does not add a Luau name. LERP does not claim that RAV
enables this option. See [GPU Shaders: Enabling GPU Canvas on Web
2.42.0](/api/gpu-shaders#enabling-gpu-canvas-on-web-2420) for the host-side
