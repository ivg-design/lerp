...
**Goal**: Apply translation and rotation transforms to a shape.

:::note Rive Result
Attach to any node and press Play. You should see a rectangle rotating around the origin as the angle updates.
:::
...
function draw(self: TransformDemo, renderer: Renderer)
    renderer:save()
    renderer:transform(Mat2D.withTranslation(0, 0))
...
**Goal**: Draw the same path multiple times with different transforms.

:::note Rive Result
You should see several copies of the same shape, each placed at a different position or scale.
:::
...
**Goal**: Update path data dynamically in `advance`.

:::note Rive Result
You should see a wavy line that animates over time as the points move.
:::
...
**Goal**: Use `clipPath` to restrict drawing to a specific area.

:::note Rive Result
You should see content drawn only inside the clipping shape; anything outside is masked.
:::
