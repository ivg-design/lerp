...
local function buildPolygon(self: Polygon)
    local n = math.floor(self.sides.value)
    if n < 3 then n = 3 end
...
function advance(self: AnimatedStar, seconds: number): boolean
    ...
    local n = math.floor(self.points.value)
    if n < 3 then n = 3 end
    local outer = self.outerRadius.value
...
function advance(self: SineWave, seconds: number): boolean
    ...
    local segs = math.floor(self.segments.value)
    local width = 300
...
function advance(self: ParticleSystem, seconds: number): boolean
    ...
    local spawnInterval = 1 / self.spawnRate.value
...
function update(self: ProgressRing)
    local r = self.radius.value
    local prog = math.clamp(self.progress.value, 0, 1)
    ...
    self.bgPaint.thickness = self.thickness.value
    self.fgPaint.thickness = self.thickness.value
end
...
function advance(self: BarChart, seconds: number): boolean
    local speed = 3
    for i, target in self.targetValues do
...
function draw(self: BarChart, renderer: Renderer)
    local barW = self.barWidth.value
    local maxH = self.maxHeight.value
    local gap = 10
