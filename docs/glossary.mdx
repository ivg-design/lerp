---
sidebar_position: 102
title: Glossary
description: Key terms and definitions for Rive scripting with Luau
keywords:
  - rive
  - luau
  - scripting
  - glossary
  - key
  - terms
  - definitions
---
import Term from '@site/src/components/Term';

# Glossary

A comprehensive reference of key terms used throughout the LERP course. Click any term to jump to its definition, and follow the "Learn more" links to dive deeper.

---

## A

### Artboard {#artboard}

The canvas in Rive where animations are created. Each <Term>Artboard</Term> can contain its own animations, state machines, and scripts. In <Term>Node</Term> scripts, you work with artboard instances via the `Artboard<T>` type.

```lua
-- Referencing another artboard as an input
export type MyScript = {
    component: Input<Artboard<Data.ComponentVM>?>,
}
```

**Learn more:** [Dynamic Instantiation](/advanced/instantiation)

---

### advance() {#advance}

A lifecycle function that runs every frame. Use it for animation updates, physics, timers, and any per-frame logic. Receives `seconds` (delta time since last frame).

```lua
function advance(self: MyScript, seconds: number): boolean
    self.timer += seconds
    return true  -- Return true to keep animating
end
```

**Learn more:** [The Lifecycle](/rive/protocols/node-lifecycle)

---

### Animation {#animation}

A timeline-based sequence of keyframes that animates properties over time. Animations are controlled by <Term>State Machine</Term>s and can be blended together.

```lua
-- Preferred flow: update ViewModel properties,
-- then let bindings/state logic react
local vm = context:viewModel()
if vm then
    local progress = vm:getNumber("progress")
    if progress then progress.value = 0.5 end  -- Control animation progress
end
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

### Array {#array}

A table with sequential integer keys starting at 1. Use `ipairs()` to iterate.

```lua
local inventory = {"sword", "shield", "potion"}

-- Access by index
print(inventory[1])  -- "sword"

-- Iterate in order
for i, item in ipairs(inventory) do
    print(`{i}: {item}`)
end

-- Get length
print(#inventory)  -- 3
```

**Learn more:** [Tables](/fundamentals/tables)

---

## B

### Bezier Curve {#bezier-curve}

A smooth curve defined by control points. Used for path drawing and easing functions.

```lua
path:moveTo(Vector.xy(0, 0))
path:cubicTo(
    Vector.xy(50, 0),    -- First control point
    Vector.xy(50, 100),  -- Second control point
    Vector.xy(100, 100)  -- End point
)
```

**Learn more:** [Procedural Graphics](/advanced/procedural)

---

### AudioSource {#audiosource}

An opaque reference to an audio asset, obtained via `context:audio(name)`. Pass to `Audio.play()` to create an [AudioSound](#audiosound).

```lua
local source = context:audio("click_sound")
if source then
    Audio.play(source)
end
```

**Learn more:** [Assets — AudioSource](/api/assets#audiosource)

---

### AudioSound {#audiosound}

A playing audio instance returned by `Audio.play()`. Provides playback control including `stop()`, `seek()`, `volume`, and `completed()`.

```lua
local sound = Audio.play(source)
sound.volume = 0.8
-- Later...
if sound:completed() then
    print("Playback finished")
end
```

**Learn more:** [Assets — AudioSound](/api/assets#audiosound)

---

### Blob {#blob}

A binary data asset accessed via `context:blob(name)`. Provides `name`, `size`, and `data` (buffer) attributes.

```lua
local blob = context:blob("myData")
if blob then
    print(blob.name, blob.size)
end
```

**Learn more:** [Assets — Blob](/api/assets#blob)

---

### Boolean {#boolean}

A type with only two values: `true` or `false`. Used for conditions and flags.

```lua
local isActive = true
local hasKey = false

if isActive and not hasKey then
    print("Active but no key")
end
```

**Learn more:** [Data Types](/fundamentals/data-types)

---

## C

### Callback {#callback}

A function passed as an argument to be called later, often in response to events.

```lua
-- Listener callback
scoreProp:addListener(function()
    print("Score changed!")
end)

-- Timer callback pattern
local function after(seconds, callback)
    -- Call callback after delay
end
```

**Learn more:** [Functions](/fundamentals/functions)

---

### Chaining {#chaining}

Calling multiple methods in sequence by returning `self` from each method.

```lua
local builder = PathBuilder.new()
builder
    :moveTo(0, 0)
    :lineTo(100, 0)
    :lineTo(100, 100)
    :close()
```

**Learn more:** [Self & Methods](/oop/self-and-methods)

---

### Closure {#closure}

A function that captures variables from its surrounding scope (called upvalues). This allows the inner function to "remember" state even after the outer function returns.

```lua
local function makeCounter()
    local count = 0  -- Captured by the closure
    return function()
        count += 1
        return count
    end
end

local counter = makeCounter()
print(counter())  -- 1
print(counter())  -- 2
```

**Learn more:** [Functions & Closures](/fundamentals/functions)

---

### Color {#color}

Rive's RGBA color type for styling fills and strokes.

```lua
-- Create colors
local red = Color.rgba(255, 0, 0, 255)
local blue = Color.rgb(0, 0, 255)
local transparent = Color.rgba(255, 255, 255, 128)

-- Access components
print(Color.red(red), Color.green(red), Color.blue(red), Color.alpha(red))
```

**Learn more:** [Core Types](/advanced/core-types)

---

### Colon Syntax {#colon-syntax}

Method call syntax that automatically passes the table as the first argument (`self`).

```lua
-- These are equivalent:
player:takeDamage(10)       -- Colon syntax
player.takeDamage(player, 10)  -- Dot syntax

-- Use colon for instance methods
function Player:takeDamage(amount)
    self.health -= amount
end
```

**Learn more:** [Self & Methods](/oop/self-and-methods)

---

### Compound Assignment {#compound-assignment}

Shorthand operators that combine assignment with an operation: `+=`, `-=`, `*=`, `/=`, `//=`, `%=`, `^=`, `..=`

```lua
local score = 100
score += 50   -- Same as: score = score + 50
score *= 2    -- Same as: score = score * 2
```

**Learn more:** [Variables & Operators](/fundamentals/variables)

---

### Context {#context}

The runtime context object passed to Node scripts, providing access to the <Term>ViewModel</Term> and other runtime features.

```lua
function init(self: MyScript, context: Context): boolean
    local vm = context:viewModel()
    if vm then
        self.scoreProp = vm:getNumber("score")
    end
    return true
end
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

## D

### DataContext {#datacontext}

Provides hierarchy traversal for data binding contexts. Obtained via `context:dataContext()`, with methods `parent()` and `viewModel()` for navigating the ViewModel hierarchy.

```lua
local dc = context:dataContext()
if dc then
    local parentDC = dc:parent()
    local vm = dc:viewModel()
end
```

**Learn more:** [Data & Input — DataContext](/api/data-input#datacontext)

---

### Data Binding {#data-binding}

The system that connects <Term>ViewModel</Term> properties to visual elements or script values. Changes propagate automatically.

```lua
-- ViewModel property bound to text in Rive Editor
-- Script can read/write the bound value
local nameProp = vm:getString("playerName")
nameProp.value = "Hero"  -- Updates the bound text automatically
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

### Delta Time {#delta-time}

The time elapsed since the last frame, passed as `seconds` to `advance()`. Essential for frame-rate-independent animations.

```lua
function advance(self: MyScript, seconds: number): boolean
    -- Move at constant speed regardless of frame rate
    self.position += self.speed * seconds
    return true
end
```

**Learn more:** [The Lifecycle](/rive/protocols/node-lifecycle)

---

### draw() {#draw}

A lifecycle function for rendering. Called on canvas repaint (not necessarily every frame). Must not modify state or rebuild paths—keep it lightweight.

```lua
function draw(self: MyScript, renderer: Renderer)
    renderer:drawPath(self.path, self.paint)
end
```

**Learn more:** [Drawing API](/advanced/drawing-api)

---

### Dictionary {#dictionary}

A table with string keys. Use `pairs()` to iterate.

```lua
local player = {
    name = "Hero",
    level = 5,
    class = "Warrior",
}

-- Access by key
print(player.name)    -- "Hero"
print(player["level"]) -- 5

-- Iterate (order not guaranteed)
for key, value in pairs(player) do
    print(`{key}: {value}`)
end
```

**Learn more:** [Tables](/fundamentals/tables)

---

### Dot Syntax {#dot-syntax}

Property or method access syntax. For methods, you must pass `self` explicitly.

```lua
-- Property access
local x = player.health

-- Method call (must pass self)
player.takeDamage(player, 10)

-- Prefer colon syntax for methods:
player:takeDamage(10)
```

**Learn more:** [Self & Methods](/oop/self-and-methods)

---

## E

### Easing {#easing}

A function that controls animation timing, making movements feel natural.

```lua
-- Common easing functions
local function easeInOut(t: number): number
    return t < 0.5
        and 2 * t * t
        or 1 - (-2 * t + 2)^2 / 2
end

local function easeOutBounce(t: number): number
    if t < 1/2.75 then
        return 7.5625 * t * t
    elseif t < 2/2.75 then
        t -= 1.5/2.75
        return 7.5625 * t * t + 0.75
    end
    -- ...
end
```

**Learn more:** [Procedural Graphics](/advanced/procedural)

---

### Event {#event}

A signal that something happened, often from user interaction or animation.

```lua
-- For pointer events in Node scripts
function pointerDown(self: MyScript, event: PointerEvent)
    local pos = event.position
    print("Pointer down at:", pos.x, pos.y)
end

-- For ViewModel triggers, use addListener
function init(self: MyScript, context: Context): boolean
    local vm = context:viewModel()
    if vm then
        local trigger = vm:getTrigger("buttonClicked")
        if trigger then
            trigger:addListener(function()
                print("Button was clicked!")
            end)
        end
    end
    return true
end
```

**Learn more:** [Listener Protocol](/rive/protocols/listener-protocol)

---

### export type {#export-type}

Declares the type of your script instance (`self`). Required for Rive's strict type checking.

```lua
export type MyScript = {
    count: number,
    name: string,
    active: boolean,
}
```

**Learn more:** [Type System Introduction](/types/intro)

---

## F

### Fill Rule {#fill-rule}

Determines how the interior of complex paths is filled. Common rules: `nonZero` (default) and `evenOdd`.

```lua
local paint = Paint.with({
    style = "fill",
    color = Color.rgb(255, 0, 0),
    fillRule = "evenOdd",  -- Alternating fill for overlapping paths
})
```

**Learn more:** [Core Types](/advanced/core-types)

---

### Function Type {#function-type}

A type annotation for functions, specifying parameters and return type.

```lua
-- Function type syntax: (params) -> returnType
type Callback = () -> ()
type Predicate = (number) -> boolean
type Transform = (number, number) -> (number, number)

-- In type definitions
export type MyScript = {
    onComplete: Callback,
    isValid: Predicate,
}
```

**Learn more:** [Custom Types](/types/custom-types)

---

### Factory Function {#factory-function}

The function returned at the end of every Node script that creates new instances. Returns `Node<YourType>`.

```lua
return function(): Node<MyScript>
    return {
        init = init,
        advance = advance,
        draw = draw,
        count = 0,           -- Default value
        name = "Player",     -- Default value
        active = late(),     -- Set in init
    }
end
```

**Learn more:** [Node Protocol](/rive/protocols/node-protocol)

---

## G

### Generic {#generic}

A type parameter that allows types and functions to work with multiple data types while maintaining type safety. Written with angle brackets: `<T>`.

```lua
-- Node<T> is a generic - T is your script type
-- Input<T> is a generic - T is the input value type

export type MyScript = {
    speed: Input<number>,      -- Input generic
    label: Input<string>,      -- Input generic
}
```

**Learn more:** [Generics](/types/generics)

---

### Gradient {#gradient}

A color gradient for <Term>Paint</Term> objects. Supports linear and radial gradients.

```lua
local gradient = Gradient.linear(
    Vector.xy(0, 0),      -- Start point
    Vector.xy(100, 0),    -- End point
    {
        { offset = 0, color = Color.rgb(255, 0, 0) },
        { offset = 1, color = Color.rgb(0, 0, 255) },
    }
)
```

**Learn more:** [Paint & Styling](/advanced/core-types#paint)

---

## I

### init() {#init}

A lifecycle function that runs once when the script starts. Use it for setup, creating paths/paints, and initializing state. Must return `true` to indicate success.

```lua
function init(self: MyScript): boolean
    self.path = Path.new()
    self.paint = Paint.with({ style = "fill", color = Color.rgb(255, 0, 0) })
    return true
end
```

**Learn more:** [The Lifecycle](/rive/protocols/node-lifecycle)

---

### Input {#input}

An editor-exposed value declared as `Input<T>`. In this course, you read it directly (e.g., `self.speed`).

```lua
export type MyScript = {
    speed: Input<number>,
    label: Input<string>,
    enabled: Input<boolean>,
}

function init(self: MyScript): boolean
    print(self.speed)  -- Read the input value
    return true
end

return function(): Node<MyScript>
    return {
        init = init,
        draw = draw,
        speed = 10,       -- Default value
        label = "Hello",  -- Default value
        enabled = true,   -- Default value
    }
end
```

**Learn more:** [Inputs & Data Binding](/rive/inputs)

---

### ImageSampler {#imagesampler}

Sampling parameters for image rendering, combining wrapping and filtering. Created with the constructor function `ImageSampler(wrapX, wrapY, filter)`.

```lua
local sampler = ImageSampler("clamp", "clamp", "bilinear")
renderer:drawImage(image, sampler, "srcOver", 1.0)
```

**Learn more:** [Assets — ImageSampler](/api/assets#imagesampler)

---

### Inheritance {#inheritance}

Creating a new class based on an existing one, reusing its properties and methods.

```lua
-- Parent class
local Shape = {}
Shape.__index = Shape

function Shape.new(x, y)
    return setmetatable({ x = x, y = y }, Shape)
end

-- Child class
local Circle = setmetatable({}, { __index = Shape })
Circle.__index = Circle

function Circle.new(x, y, radius)
    local self = Shape.new(x, y)
    self.radius = radius
    return setmetatable(self, Circle)
end
```

**Learn more:** [Inheritance](/oop/inheritance)

---

### Intersection Type {#intersection-type}

Combines multiple types with `&`. The result has all properties from both types.

```lua
type Drawable = { draw: (Drawable, Renderer) -> () }
type Movable = { move: (Movable, number, number) -> () }
type Sprite = Drawable & Movable  -- Has both draw AND move
```

**Learn more:** [Advanced Types](/types/advanced-types)

---

### ipairs {#ipairs}

An iterator for arrays that yields index-value pairs in order (1, 2, 3...).

```lua
local items = {"sword", "shield", "potion"}
for i, item in ipairs(items) do
    print(`{i}: {item}`)
end
```

**Learn more:** [Tables](/fundamentals/tables)

---

## K

### Keyframe {#keyframe}

A specific point in time that defines a property's value in an <Term>Animation</Term>. The animation system interpolates between keyframes to create smooth transitions.

**Learn more:** [Animation](#animation)

---

## L

### Layer {#layer}

In Rive <Term>State Machine</Term>s, layers allow multiple animations to play simultaneously. Each layer can have its own states and transitions, enabling complex animation blending.

**Learn more:** [State Machine](#state-machine)

---

### Lerp {#lerp}

Short for **L**inear int**ERP**olation. A fundamental technique for smoothly transitioning between two values.

```lua
-- Linear interpolation: blend from a to b by factor t
local function lerp(a: number, b: number, t: number): number
    return a + (b - a) * t
end

-- Examples
lerp(0, 100, 0)    -- 0 (start)
lerp(0, 100, 0.5)  -- 50 (middle)
lerp(0, 100, 1)    -- 100 (end)
```

**Learn more:** [Procedural Graphics](/advanced/procedural)

---

### ListenerAction {#listeneraction}

A script type for custom logic that runs when a state machine listener event fires. Uses `init` and `performAction` as the preferred callback (`perform` is legacy/deprecated).

```lua
return function(): ListenerAction<MyAction>
    return { init = init, performAction = performAction }
end
```

**Learn more:** [ListenerAction Protocol](/rive/protocols/listener-action-protocol)

---

### Listener {#listener}

A <Term>Callback</Term> function registered to respond when a value changes or an event occurs.

```lua
-- Add listener to ViewModel property
local scoreProp = vm:getNumber("score")
scoreProp:addListener(function()
    print(`Score is now: {scoreProp.value}`)
end)

-- Listeners fire whenever the value changes
scoreProp.value = 100  -- Triggers: "Score is now: 100"
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

### Local Variable {#local-variable}

A variable declared with the `local` keyword, scoped to the current block. Always prefer `local` over globals.

```lua
local x = 10        -- Local to this scope
local y, z = 1, 2   -- Multiple declarations

do
    local inner = 5  -- Only visible in this block
end
-- inner is not accessible here
```

**Learn more:** [Variables & Operators](/fundamentals/variables)

---

### Loop {#loop}

Control structures for repeated execution. Luau has `for`, `while`, and `repeat-until` loops.

```lua
-- Numeric for loop
for i = 1, 5 do
    print(i)  -- 1, 2, 3, 4, 5
end

-- Iterator for loop
for i, item in ipairs(items) do
    print(item)
end

-- While loop
while condition do
    -- runs while condition is true
end

-- Repeat-until loop
repeat
    -- runs at least once
until condition
```

**Learn more:** [Control Flow](/fundamentals/control-flow)

---

### late() {#late}

A type-safe placeholder for properties that will be set in `init()` rather than in the factory function. Recommended (not required) for <Term>Path</Term>, Paint, and computed values for type safety.

```lua
export type MyScript = {
    path: Path,
    paint: Paint,
    computed: number,
}

return function(): Node<MyScript>
    return {
        init = init,
        draw = draw,
        path = late(),      -- Type-safe placeholder for init()
        paint = late(),     -- Type-safe placeholder for init()
        computed = late(),  -- Type-safe placeholder for init()
    }
end
```

**Note:** `nil` also works at runtime, but `late()` provides type safety in strict mode.

**Learn more:** [The late() Initializer](/types/late-initializer)

---

### Lifecycle {#lifecycle}

The sequence of functions Rive calls on your script: `init` → `update` → `advance` → `draw`.

| Function | When Called | Purpose |
|----------|-------------|---------|
| `init` | Once at start | Setup, create objects |
| `update` | When inputs change | React to <Term>Input</Term> changes |
| `advance` | Every animation frame | Animation, physics |
| `draw` | On canvas repaint | Rendering (pure, no state changes) |

**Learn more:** [The Lifecycle](/rive/protocols/node-lifecycle)

---

## M

### Mat2D {#mat2d}

A 2D transformation matrix for translation, rotation, scale, and skew.

```lua
-- Create transforms
local translate = Mat2D.withTranslation(100, 50)
local rotate = Mat2D.withRotation(math.pi / 4)  -- 45 degrees
local scale = Mat2D.withScale(2, 2)

-- Combine transforms
local combined = translate * rotate

-- Apply to renderer
renderer:transform(combined)
```

**Learn more:** [Transforms & <Term>Renderer</Term>](/advanced/drawing-api)

---

### Metatable {#metatable}

A table that defines special behavior for another table, such as how it responds to operators or property access. Set with `setmetatable()`.

```lua
local mt = {
    __index = function(tbl, key)
        return "default"
    end
}
local t = setmetatable({}, mt)
print(t.anything)  -- "default"
```

**Learn more:** [Metatables](/oop/metatables)

---

### Metamethod {#metamethod}

A function in a metatable that defines special behavior. Common metamethods:

| Metamethod | Triggered By |
|------------|--------------|
| `__index` | Reading missing keys |
| `__newindex` | Writing missing keys |
| `__add` | The `+` operator |
| `__tostring` | Converting to string |
| `__call` | Calling as a function |

**Learn more:** [The __index Metamethod](/oop/index-metamethod)

---

### Module {#module}

A reusable chunk of code that can be imported with `require()`. In Rive, <Term>Util Protocol</Term> scripts are modules.

```lua
-- math_utils.lua (Util Script)
local MathUtils = {}

function MathUtils.clamp(value: number, min: number, max: number): number
    return math.max(min, math.min(max, value))
end

return MathUtils

-- In another script
local MathUtils = require("math_utils")
local clamped = MathUtils.clamp(150, 0, 100)  -- 100
```

**Learn more:** [Util Scripts](/rive/protocols/util-protocol)

---

## N

### Nested Artboard {#nested-artboard}

An <Term>Artboard</Term> instance placed inside another artboard. Allows reusable components with their own animations and scripts.

```lua
export type MyScript = {
    -- Reference to a nested artboard's ViewModel
    component: Input<Artboard<Data.ButtonVM>?>,
}

function init(self: MyScript): boolean
    if self.component then
        -- Access the nested artboard's ViewModel
        local vm = self.component:viewModel()
    end
    return true
end
```

**Learn more:** [Dynamic Instantiation](/advanced/instantiation)

---

### nil {#nil}

Luau's value representing "nothing" or "no value". Variables are `nil` by default, and setting a table key to `nil` removes it.

```lua
local x          -- x is nil
local y = nil    -- Explicit nil

local t = { a = 1 }
t.a = nil        -- Removes key 'a'

-- Check for nil
if value == nil then
    print("Value is nil")
end

-- Shorthand (nil is falsy)
if not value then
    print("Value is nil or false")
end
```

**Learn more:** [Data Types](/fundamentals/data-types)

---

### Node Protocol {#node-protocol}

The primary Rive script type that attaches to scene nodes. Implements lifecycle functions (`init`, `advance`, `draw`) and can perform custom rendering.

```lua
export type MyScript = {}

function init(self: MyScript): boolean
    return true
end

function draw(self: MyScript, renderer: Renderer)
end

return function(): Node<MyScript>
    return { init = init, draw = draw }
end
```

**Learn more:** [Node Protocol](/rive/protocols/node-protocol)

---

## O

### Output {#output}

`Output<T>` appears in upstream docs, but at the current C++ runtime baseline it is not exposed in Luau bindings. Use normal typed fields on `self` for script state until a future runtime baseline confirms support.

```lua
export type MyScript = {
    result: number,
}
```

**Learn more:** [System — Output](/api/system#output)

---

### Optional Type {#optional-type}

A type that can be `nil`, denoted with `?`. Use for values that might not exist.

```lua
export type MyScript = {
    target: Node?,        -- Might be nil
    backup: string?,      -- Might be nil
}

-- Must check before using
if self.target then
    -- Safe to use self.target here
end
```

**Learn more:** [Custom Types](/types/custom-types)

---

## P

### pairs {#pairs}

An iterator for tables that yields key-value pairs. Works with any table (arrays, dictionaries, mixed). Order is not guaranteed.

```lua
local player = { name = "Hero", level = 5, class = "Warrior" }

for key, value in pairs(player) do
    print(`{key}: {value}`)
end
-- Output (order may vary):
-- name: Hero
-- level: 5
-- class: Warrior
```

**Learn more:** [Tables](/fundamentals/tables)

---

### Paint {#paint}

Rive's styling object for fills and strokes. Controls color, stroke width, cap style, and more.

```lua
-- Fill paint
local fill = Paint.with({
    style = "fill",
    color = Color.rgb(255, 0, 0),
})

-- Stroke paint
local stroke = Paint.with({
    style = "stroke",
    thickness = 3,
    color = Color.rgb(0, 0, 255),
    cap = "round",
    join = "round",
})
```

**Learn more:** [Core Types](/advanced/core-types)

---

### Path {#path}

Rive's vector path type for creating custom shapes.

```lua
local path = Path.new()
path:moveTo(Vector.xy(0, 0))
path:lineTo(Vector.xy(100, 0))
path:lineTo(Vector.xy(100, 100))
path:lineTo(Vector.xy(0, 100))
path:close()
```

**Learn more:** [Core Types](/advanced/core-types)

---

### PointerEvent {#pointerevent}

Event data for pointer interactions (mouse, touch).

```lua
function pointerDown(self: MyScript, event: PointerEvent)
    print(`Pointer at: {event.position.x}, {event.position.y}`)
    event:hit()  -- Consume the event
end
```

**Learn more:** [Game Logic & Pointer Events](/advanced/game-logic)

---

### Property {#property}

A mutable ViewModel property. Can be read, written, and listened to.

```lua
-- Get property from ViewModel
local scoreProp = vm:getNumber("score")

-- Read value
local score = scoreProp.value

-- Write value
scoreProp.value = score + 10

-- Listen for changes
scoreProp:addListener(function()
    print(`Score changed to: {scoreProp.value}`)
end)
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

## R

### require {#require}

Imports a <Term>Module</Term> by name. In Rive, use `require()` to load <Term>Util Protocol</Term> scripts.

```lua
-- Import a utility module
local MathUtils = require("math_utils")
local Colors = require("colors")

-- Use the imported module
local clamped = MathUtils.clamp(150, 0, 100)
local primary = Colors.primary
```

**Learn more:** [Util Scripts](/rive/protocols/util-protocol)

---

### Return {#return}

The keyword that exits a function and optionally provides a value. Luau functions can return multiple values.

```lua
-- Single return
local function double(x: number): number
    return x * 2
end

-- Multiple returns
local function minMax(a: number, b: number): (number, number)
    if a < b then
        return a, b
    else
        return b, a
    end
end

local min, max = minMax(5, 3)  -- min=3, max=5

-- Early return for guard clauses
local function process(data: Data?)
    if not data then return end  -- Exit early
    -- Process data...
end
```

**Learn more:** [Functions](/fundamentals/functions)

---

### Renderer {#renderer}

The drawing context provided to `draw()` for custom rendering. Works like a <Term>State Machine</Term> with save/restore.

```lua
function draw(self: MyScript, renderer: Renderer)
    renderer:save()                           -- Save state
    renderer:transform(Mat2D.withTranslation(50, 50))
    renderer:drawPath(self.path, self.paint)
    renderer:restore()                        -- Restore state
end
```

**Learn more:** [Drawing API](/advanced/drawing-api)

---

## S

### self {#self}

The script instance. Contains your persistent state and is passed to all lifecycle functions.

```lua
function init(self: MyScript): boolean
    self.count = 0            -- Store state on self
    return true
end

function advance(self: MyScript, seconds: number): boolean
    self.count += 1           -- Access state from self
    return true
end
```

**Learn more:** [Self & Methods](/oop/self-and-methods)

---

### Scope {#scope}

The region of code where a variable is visible and accessible. Luau has block scope—variables exist only within their declaring block.

```lua
local global = "visible everywhere"

if true then
    local inner = "only in this block"
    print(global)  -- Works
    print(inner)   -- Works
end

print(global)      -- Works
-- print(inner)    -- Error: inner is out of scope
```

**Learn more:** [Variables & Operators](/fundamentals/variables)

---

### State Machine {#state-machine}

Rive's logic layer that controls which animations play and when. State Machines contain inputs (Boolean, Number, Trigger), layers, states, and transitions with conditions. Scripts can interact with State Machines through ViewModel data binding.

```lua
-- Scripts don't directly access State Machines
-- Instead, bind ViewModel properties to State Machine inputs
-- Then read/write those properties from scripts

local vm = context:viewModel()
if vm then
    -- This ViewModel property is bound to a State Machine input
    local health = vm:getNumber("health")
    if health then health.value = 75 end
end
```

**Learn more:** [ViewModel Integration](/advanced/viewmodels)

---

### Strict Mode {#strict-mode}

Rive enforces strict type checking by default. This mode activates Luau's type checker, catching errors at compile time rather than runtime.

```lua
export type MyScript = {
    count: number,  -- Type required
}
```

**Learn more:** [Strict Mode](/types/strict-mode)

---

### String Interpolation {#string-interpolation}

Luau's syntax for embedding expressions inside strings using backticks and curly braces.

```lua
local name = "Hero"
local level = 5
local damage = 12.5

-- Interpolate variables
print(`Welcome, {name}!`)           -- Welcome, Hero!

-- Interpolate expressions
print(`Level {level} ({level * 10} XP)`)  -- Level 5 (50 XP)

-- Format numbers
print(`Damage: {damage:.1f}`)        -- Damage: 12.5
```

**Learn more:** [Data Types](/fundamentals/data-types)

---

## T

### Table {#table}

Luau's only data structure—a versatile container that can be an <Term>Array</Term>, <Term>Dictionary</Term>, or both.

```lua
-- Array (sequential integer keys)
local items = {"sword", "shield", "potion"}

-- Dictionary (string keys)
local player = { name = "Hero", level = 5 }

-- Mixed (both)
local mixed = {
    [1] = "first",
    name = "example",
}

-- Nested tables
local game = {
    player = { x = 0, y = 0 },
    enemies = { { x = 10, y = 5 }, { x = 20, y = 15 } },
}
```

**Learn more:** [Tables](/fundamentals/tables)

---

### Truthiness {#truthiness}

How values behave in boolean contexts. In Luau, only `false` and `nil` are **falsy**—everything else is **truthy** (including `0` and `""`).

```lua
-- Falsy values (only these two)
if not false then print("false is falsy") end
if not nil then print("nil is falsy") end

-- Truthy values (everything else)
if 0 then print("0 is truthy!") end
if "" then print("empty string is truthy!") end
if {} then print("empty table is truthy!") end

-- Common pattern: nil check
local name = getData() or "default"  -- Use "default" if getData() returns nil
```

**Learn more:** [Control Flow](/fundamentals/control-flow)

---

### TransitionCondition {#transitioncondition}

A script type for custom state machine transition evaluation logic. Uses `init` and `evaluate` protocol methods. Returns true/false to allow/block transitions.

```lua
return function(): TransitionCondition<MyCondition>
    return { init = init, evaluate = evaluate }
end
```

**Learn more:** [TransitionCondition Protocol](/rive/protocols/transition-condition-protocol)

---

### Trigger {#trigger}

A ViewModel trigger that fires events without storing a value.

```lua
-- Get trigger from ViewModel
local clickTrigger = vm:getTrigger("onClick")

-- Fire the trigger
clickTrigger:fire()

-- Listen for trigger fires
clickTrigger:addListener(function()
    print("Trigger fired!")
end)
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

### Type Alias {#type-alias}

A name for a type, created with the `type` keyword.

```lua
type Position = { x: number, y: number }
type State = "idle" | "walking" | "running"
type Callback = (number) -> ()
```

**Learn more:** [Custom Types](/types/custom-types)

---

### Type Guard {#type-guard}

A runtime check that narrows a variable's type. Used to safely access properties of union or optional types.

```lua
-- Using typeof
local function describe(value: string | number): string
    if typeof(value) == "string" then
        return `String: {value}`  -- value is string here
    else
        return `Number: {value}`  -- value is number here
    end
end

-- Nil check narrows optional types
local function greet(name: string?)
    if name then
        print(`Hello, {name}!`)  -- name is string here
    end
end
```

**Learn more:** [Advanced Types](/types/advanced-types)

---

### Type Narrowing {#type-narrowing}

The process where the type checker automatically restricts a variable's type based on control flow (if statements, <Term>Type Guard</Term>s).

```lua
local value: string | number = getData()

-- Before check: value is string | number
if typeof(value) == "string" then
    -- Narrowed: value is string
    print(value:upper())
else
    -- Narrowed: value is number
    print(value * 2)
end
```

**Learn more:** [Advanced Types](/types/advanced-types)

---

## U

### Union Type {#union-type}

A type that can be one of several types, using `|`.

```lua
type State = "idle" | "walking" | "running"
type Result = number | string | nil
```

**Learn more:** [Custom Types](/types/custom-types)

---

### update() {#update}

A lifecycle function that runs when input values change. Use it to react to editor changes or runtime data binding updates.

```lua
function update(self: MyScript)
    -- Called when any Input<T> value changes
    self.path:reset()
    self.rebuildPath()
end
```

**Learn more:** [The Lifecycle](/rive/protocols/node-lifecycle)

---

### Upvalue {#upvalue}

A variable from an outer scope that is captured by a closure.

```lua
local multiplier = 2  -- This becomes an upvalue

local function double(x)
    return x * multiplier  -- Captures 'multiplier'
end
```

**Learn more:** [Functions & Closures](/fundamentals/functions)

---

### Util Protocol {#util-protocol}

A Rive script type for creating reusable modules. Cannot draw or have lifecycle functions—just exports values and functions.

```lua

local MathUtils = {}

function MathUtils.clamp(value: number, min: number, max: number): number
    return math.max(min, math.min(max, value))
end

function MathUtils.lerp(a: number, b: number, t: number): number
    return a + (b - a) * t
end

return MathUtils
```

**Learn more:** [Util Scripts](/rive/protocols/util-protocol)

---

## V

### Vector {#vector}

Rive's 2D vector type (alias: `Vec2D`). Used for positions, directions, and sizes.

```lua
-- Create vectors
local pos = Vector.xy(100, 50)
local zero = Vector.origin()

-- Operations
local sum = pos + Vector.xy(10, 20)
local scaled = pos * 2
local length = pos:length()
local normalized = pos:normalized()
```

**Learn more:** [Core Types](/advanced/core-types)

---

### ViewModel {#viewmodel}

Rive's data binding container that allows properties to be shared between scripts, state machines, and external code.

```lua
function init(self: MyScript, context: Context): boolean
    local vm = context:viewModel()
    if vm then
        self.scoreProp = vm:getNumber("score")
        self.nameProp = vm:getString("playerName")
    end
    return true
end
```

**Learn more:** [ViewModels](/advanced/viewmodels)

---

### Mat4 {#mat4}

Rive's 4x4 matrix type for 3D transforms, projection math, and GPU-oriented workflows.

```lua
local model = Mat4.fromTranslation(0, 0, -4) * Mat4.fromRotationY(math.rad(20))
local x, y, z, w = model:transformVec4(1, 0, 0, 1)
```

**Learn more:** [Core Types](/api/core-types#mat4)

---

### Promise {#promise}

An async value container used with `Promise.new`, chaining (`andThen`, `catch`, `finally`), and cancellation APIs.

```lua
local p = Promise.resolve(42)
p:andThen(function(v) print(v) end)
```

**Learn more:** [System](/api/system#promise-async-and-await)

---

### ScriptedInterpolator {#scripted-interpolator}

A runtime-backed interpolation protocol with optional `transform` and `transformValue` hooks for custom easing behavior.

```lua
function transformValue(self, from: number, to: number, factor: number): number
    local t = factor * factor
    return from + (to - from) * t
end
```

If omitted or failing, runtime falls back to linear interpolation.

**Learn more:** [ScriptedInterpolator Protocol](/rive/protocols/scripted-interpolator-protocol)

---

### drawCanvas() {#drawcanvas}

An optional Node/Layout lifecycle callback for offscreen `Canvas` and early-access `GPUCanvas` rendering. Use it for `Canvas:beginFrame(...)` and `GPUCanvas:beginRenderPass(...)`; use `draw(self, renderer)` afterward to composite the resulting image.

```lua
function drawCanvas(self: ShaderNode)
    local pass = self.gpuCanvas:beginRenderPass({
        color = {{ loadOp = "clear", storeOp = "store", clearColor = { 0, 0, 0, 0 } }},
    })
    pass:setPipeline(self.pipeline)
    pass:draw(self.vertexCount)
    pass:finish()
end
```

**Learn more:** [GPU Shaders and 3D Rendering](/advanced/gpu-shaders)

---

### GPUCanvas {#gpucanvas}

A GPU render target created with `context:gpuCanvas(...)`. It exposes `beginRenderPass(...)` for shader rendering and `image` for normal renderer compositing.

```lua
self.gpuCanvas = context:gpuCanvas({ width = 512, height = 512 })
local format = self.gpuCanvas.format
```

**Learn more:** [GPU Shaders API](/api/gpu-shaders#canvas-vs-gpucanvas)

---

### Bind Group {#bind-group}

A reusable set of shader resources: uniform buffers, textures, and samplers. Bind groups map Luau resource entries to WGSL `@group` and `@binding` declarations.

```lua
local bindGroup = GPUBindGroup.new({
    layout = pipeline:getBindGroupLayout(0),
    ubos = {{ slot = 0, buffer = uniformBuffer }},
    textures = {{ slot = 1, view = image:view() }},
    samplers = {{ slot = 2, sampler = sampler }},
})
```

**Learn more:** [GPU Shaders API](/api/gpu-shaders#bind-groups)

---

### Shader {#shader}

An opaque compiled GPU shader module loaded from a `.wgsl` asset with `context:shader(name)`.

```lua
local shader = context:shader("gradient_card")
if not shader then
    print("Missing shader asset")
end
```

**Learn more:** [GPU Shaders API](/api/gpu-shaders#shader)

---

### WGSL {#wgsl}

WebGPU Shading Language. Rive shader assets are authored as `.wgsl` files and loaded from Luau as compiled `Shader` handles.

```text
@fragment
fn fsMain() -> @location(0) vec4<f32> {
    return vec4<f32>(1.0, 0.2, 0.6, 1.0);
}
```

**Learn more:** [GPU Shaders and 3D Rendering](/advanced/gpu-shaders)

---

## Quick Reference Index

| Category | Terms |
|----------|-------|
| **Lifecycle** | [init](#init), [update](#update), [advance](#advance), [drawCanvas](#drawcanvas), [draw](#draw), [Lifecycle](#lifecycle) |
| **Types** | [export type](#export-type), [Generic](#generic), [Union Type](#union-type), [Optional Type](#optional-type), [Intersection Type](#intersection-type), [Type Alias](#type-alias), [Type Guard](#type-guard), [Type Narrowing](#type-narrowing) |
| **Data** | [Input](#input), [Property](#property), [ViewModel](#viewmodel), [Trigger](#trigger), [Output](#output), [late()](#late), [Data Binding](#data-binding), [DataContext](#datacontext), [Listener](#listener) |
| **Drawing** | [Path](#path), [Paint](#paint), [Renderer](#renderer), [Color](#color), [Gradient](#gradient), [Mat2D](#mat2d), [Mat4](#mat4), [Vector](#vector), [ImageSampler](#imagesampler) |
| **GPU Shaders** | [Shader](#shader), [GPUCanvas](#gpucanvas), [Bind Group](#bind-group), [WGSL](#wgsl), [Mat4](#mat4) |
| **OOP** | [Metatable](#metatable), [Metamethod](#metamethod), [Closure](#closure), [self](#self), [Upvalue](#upvalue), [Inheritance](#inheritance), [Chaining](#chaining) |
| **Script Types** | [Node Protocol](#node-protocol), [Util Protocol](#util-protocol), [ListenerAction](#listeneraction), [TransitionCondition](#transitioncondition), [ScriptedInterpolator](#scripted-interpolator), [Factory Function](#factory-function), [Module](#module), [require](#require) |
| **Async** | [Promise](#promise), [Listener](#listener), [Output](#output) |
| **Tables** | [Table](#table), [Array](#array), [Dictionary](#dictionary), [pairs](#pairs), [ipairs](#ipairs) |
| **Language** | [Local Variable](#local-variable), [Scope](#scope), [Loop](#loop), [Return](#return), [nil](#nil), [Boolean](#boolean), [Truthiness](#truthiness), [String Interpolation](#string-interpolation), [Compound Assignment](#compound-assignment) |
| **Animation** | [Animation](#animation), [Keyframe](#keyframe), [State Machine](#state-machine), [Layer](#layer), [Lerp](#lerp), [Easing](#easing), [Artboard](#artboard), [Nested Artboard](#nested-artboard), [AudioSource](#audiosource), [AudioSound](#audiosound), [Blob](#blob) |
| **Syntax** | [Colon Syntax](#colon-syntax), [Dot Syntax](#dot-syntax), [Strict Mode](#strict-mode) |

## Next Steps

- Continue to [Welcome to LERP](/)
- Refresh syntax in [Quick Reference](/quick-reference)
