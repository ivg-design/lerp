...
if vm then
    vm:setNumber("progress", 0.5)  -- Control animation progress
end
```
...
```lua
-- Rive events from State Machine
function init(self: MyScript, context: Context): boolean
    -- Listen for Rive events
    context:addEventListener("buttonClicked", function()
...
A special Rive initializer for properties that will be set in `init()` rather than in the factory function. Required for Rive types like <Term>Path</Term>, Paint, and computed values.
