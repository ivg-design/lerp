...
function init(self: MyScript, context: Context): boolean
    ...
    if vm then
        self.scoreProp = vm:property("score")
    end
    return true
end
```
...
local gradient = Gradient.linear(
    ...
    {
        { offset = 0, color = Color.hex("#FF0000") },
        { offset = 1, color = Color.hex("#0000FF") },
    }
)
```
...
function init(self: MyScript): boolean
    self.path = Path.new()
    self.paint = Paint.with({ style = "fill", color = Color.hex("#FF0000") })
    return true
end
```
...
local scoreProp = vm:property("score")

-- Read value
local score = scoreProp:get()

-- Write value
scoreProp:set(score + 10)

-- Listen for changes
scoreProp:addListener(function(newValue)
...
local clickTrigger = vm:trigger("onClick")
...
function init(self: MyScript, context: Context): boolean
    ...
    if vm then
        self.scoreProp = vm:property("score")
        self.nameProp = vm:property("playerName")
    end
    return true
end
```
