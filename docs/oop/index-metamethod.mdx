...
`__newindex` only fires for assignments to keys that are missing in the target table.
To make config effectively read-only (including existing keys), expose a proxy table:

- `__index` reads from a backing source table
- `__newindex` ignores writes on the proxy

:::info Goal
By the end of this exercise, you will build a read-only proxy with `__index` and `__newindex`.
:::

### Use Case

You want runtime code to read config values but never mutate them directly.
The proxy pattern prevents changes to both existing and new keys.
...
function init(self: Exercise6): boolean
    ...
    setmetatable(config, {
        ...
        __index = nil,
        -- TODO: Ignore writes on the proxy
        __newindex = function(t, key, value)
        end,
    })

    -- Existing key in source: should still be blocked
    config.mode = "open"
    -- New key on proxy: should also be blocked
    config.debug = true
...
1. Set `__index` to `source`
2. Make `__newindex` ignore writes
3. Keep the original `mode` value
...
- [ ] Proxy reads from source via `__index`
- [ ] Writes are ignored by `__newindex`
- [ ] Mode remains locked
- [ ] Output matches the expected line
...
### 5. Expecting __newindex for Existing Keys on the Same Table

```lua
-- WRONG assumption: existing key bypasses __newindex
local t = { mode = "locked" }
setmetatable(t, {
    __newindex = function() print("blocked") end
})
t.mode = "open"  -- __newindex NOT called

-- CORRECT: use a proxy table
local source = { mode = "locked" }
local proxy = {}
setmetatable(proxy, {
    __index = source,
    __newindex = function() end, -- ignore writes
})
proxy.mode = "open"  -- blocked
```
