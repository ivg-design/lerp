...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  --!strict                          │ Enable type checking (REQUIRED)       │
├─────────────────────────────────────────────────────────────────────────────┤
│  export type MyNode = {             │ Define your script's data shape       │
│      path: Path,                    │   - Rive objects (Path, Paint)        │
│      time: number,                  │   - State variables                   │
│      speed: Input<number>,          │   - Editor inputs (Input<T>)          │
│  }                                  │                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│  function init(self: MyNode)        │ Runs ONCE at startup                  │
│      -- Create objects here         │   - Create Path, Paint objects        │
│      return true                    │   - Initialize state                  │
│  end                                │   - Return true to continue           │
├─────────────────────────────────────────────────────────────────────────────┤
│  function advance(self, seconds)    │ Runs EVERY FRAME (optional)           │
│      -- Update logic here           │   - Animation, physics                │
│      return true                    │   - Return true to keep running       │
│  end                                │                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│  function draw(self, renderer)      │ Runs EVERY FRAME (REQUIRED)           │
│      -- Render here                 │   - Draw paths with renderer          │
│  end                                │   - Apply transforms                  │
├─────────────────────────────────────────────────────────────────────────────┤
│  return function(): Node<MyNode>    │ Factory function (REQUIRED)           │
│      return {                       │   - Creates fresh instance per node   │
│          init = init,               │   - Wire up lifecycle functions       │
│          advance = advance,         │   - Set Input<T> defaults             │
│          draw = draw,               │   - Use late() for runtime values     │
│          speed = 100,               │                                       │
│          path = late(),             │                                       │
│      }                              │                                       │
│  end                                │                                       │
...
| Transforms | [Drawing API](/advanced/drawing-api) (Mat2D, <Term>Renderer</Term>) |
...
**Why it's required:** Rive Node scripts MUST start with `--!strict` to be recognized. Without it, Rive won't treat the file as a script.
...
**What it does:** `late()` is a placeholder that says "this value will be set later, in `init()`."

**Why it exists:** The factory function runs BEFORE your script is fully attached to a node. At that moment:
- You can't create Rive objects like `Path.new()` or `Paint.new()` yet
- You don't know the actual input values from the Editor
- You're just creating the data structure, not running the script

So `late()` is like writing "TBD" — it satisfies the type checker while promising you'll fill in the real value in `init()`.
...
| Everything else | `late()` | Created in `init()` |
