...
| `function init(self, context)` | `constructor()` |
...
| Topic | Section |
|-------|---------|
| Full lifecycle details | [Node Lifecycle Protocol](/rive/protocols/node-lifecycle) |
...
In Rive, a **Node** is any element in your hierarchy - groups, paths, images, text, shapes, script nodes, etc.

When you add a Node script to the scene, Rive creates a **script node** in the artboard hierarchy. You can place that script node at the artboard root, move it on the canvas, or nest it under a group or shape. Its position, rotation, scale, opacity, draw order, and inherited parent transforms work like normal scene hierarchy behavior.

Nesting a Node script under a shape or group does **not** give the script a special "this shape" object or extra access to that parent's geometry. A Node script gets its documented lifecycle parameters: `context` in `init`, `renderer` in `draw`, `seconds` in `advance`, and pointer events in pointer callbacks. To read or modify an existing shape's path data, use a <Term>Path</Term> Effect script. To access a specific scene node from a Node script, use an explicit reference such as `artboard:node(name)` or an editor input when that pattern is available.

**To add a Node script to the scene:**
1. Create a script asset in the Assets panel
2. Add the script to the artboard
3. Position it or nest it in the hierarchy where its transform and draw order should live
...
### 1. `export type MyScript = {}` — Describing Your Data
...
### 2. `function init(self: MyScript, context: Context): boolean` — Running Once at Startup
...
| `context: Context` | Runtime context with access to ViewModels and assets |
...
### 3. `function draw(self: MyScript, renderer: Renderer)` — Rendering on Repaint
...
**Even if empty, it must exist!** Rive needs this function to recognize your script as a Node script. Without it, you can't add the script to the scene as a Node script.
...
### 4. `return function(): Node<MyScript>` — The Factory Function
...
**Why it exists:** You might use your script asset in 10 different places. Each script node needs its own separate data. The factory function creates a fresh copy for each.
...
function init(potato: MyScript, context: Context): boolean
