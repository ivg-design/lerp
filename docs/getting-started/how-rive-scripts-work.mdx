...
| Code | Purpose |
|------|---------|
| `--!strict` | **Enable type checking (RECOMMENDED)** |
...
| `function advance(self, seconds) ... end` | **Runs EVERY FRAME (optional)** — Animation, physics, return true to keep running |
| `function draw(self, renderer) ... end` | **Runs EVERY FRAME (REQUIRED)** — Draw paths with renderer, apply transforms |
...
**Why it's recommended:** While scripts run without `--!strict`, using it enables full type checking which catches errors before runtime. It's strongly recommended for production scripts.
