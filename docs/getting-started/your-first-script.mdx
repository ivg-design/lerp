...
<Quiz
  ...
  id="beginner-q7"
  options={[
    { text: "Makes the script run faster" },
    { text: "Turns on checking that catches mistakes early", correct: true },
    { text: "Adds strict security" },
    { text: "Nothing, it's just a comment" },
  ]}
  explanation="--!strict enables the type checker, which analyzes your code and warns you about potential problems before you run it."
/>
...
<ParsonsProblem
  ...
  blocks={[
    "--!strict",
    "export type MyScript = { }",
    "function init(self: MyScript): boolean ... end",
    "function draw(self: MyScript, renderer: Renderer) ... end",
    "return function(): Node<MyScript> ... end"
  ]}
  hint="Start with strict mode, then type definition, then the functions, and finally the factory."
  explanation="Rive scripts follow this order: strict mode directive, type definition, lifecycle functions, and finally the factory function that returns everything."
/>

---

## A Real Example: Pulsing Circle

Here's a complete script that draws a pulsing circle. Notice how each part we learned fits together:

```lua
--!strict

export type PulsingCircle = {
    time: number,           -- Tracks elapsed time
    size: number,           -- Current circle size
}

function init(self: PulsingCircle): boolean
    self.time = 0           -- Start at time 0
    self.size = 50          -- Start at size 50
    print("Circle ready!")
    return true             -- Success!
end

function advance(self: PulsingCircle, elapsed: number): boolean
    self.time = self.time + elapsed    -- Add time that passed
    self.size = 50 + math.sin(self.time * 3) * 20  -- Pulse between 30 and 70
    return true
end
...
Now that you understand the basics, you have two paths:

- **Continue learning step by step:** Go to [Variables, Types & Operators](/fundamentals/variables) to deepen your Luau knowledge
- **See more script structure details:** Check [How Rive Scripts Work](/getting-started/how-rive-scripts-work) for comparisons to JavaScript and After Effects
