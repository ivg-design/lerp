...
- Direct Web runtime line: Canvas, Canvas Lite, WebGL2, and Canvas Advanced
  `2.42.0`, backed by C++ `runtime-v0.1.344`
...
- Released Web package gitHead: `cb8b0d6a679746c77e33bee3f47a0fb749e25b9c`
- Released tag, canary, and main runtime refs agree on the accepted commit
- Shader/GPU rollout tier: Early Access editor workflow plus Web `2.42.0`'s
  experimental, default-off `enableGPUCanvas` host option
- Baseline revalidated: September 4, 2026
- Released coverage: global ViewModels, optional three-argument Layout resize
  ...
  focus, nested-artboard origin/layout behavior, and Web `2.42.0`'s preferred
  singular `stateMachine` selection. Web GPU Canvas remains an explicit host
  opt-in and does not change the runtime `344` Luau surface
...
Web `2.42.0` exposes `enableGPUCanvas` as an experimental JavaScript import
