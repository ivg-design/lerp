...
Rive scripts support `--!strict` mode for type safety. While scripts can run without it, strict mode is strongly recommended because it ensures:
...
<Quiz
  ...
  type="code"
  answer="--!strict"
  explanation="The --!strict directive enables full type checking. Place it near the top of your script (it doesn't have to be the first line). Strongly recommended for catching errors early."
/>
...
1. **Missing `--!strict`**: Add it near the top of your file for type checking (recommended, not required)
   ```lua
   --!strict  -- Recommended at top of file
