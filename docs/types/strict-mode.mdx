...
description: Master strict mode and understand the type errors it catches
keywords:
  - rive
  - luau
  - scripting
...
- Know why Rive enforces strict typing
...
| Strict mode | N/A | Enforced by Rive | Rive enforces strict typing by default |
| TypeScript | `tsc --strict` | Strict typing | Similar concept |
...
```lua
-- Luau strict mode - error shown in editor BEFORE running
...
Rive enforces strict type checking by default. This design ensures:
...
## Type Checking in Rive

Rive enforces strict type checking by default. This means full type inference, errors on type mismatches, and required annotations for functions.
...
1. **Empty tables without types**: `local t: {string} = {}` not `local t = {}`

2. **Plain tables for Rive types**: Use `Vector.xy()` not `{x = 100, y = 50}`

3. **Not handling optionals**: Always check `string?` before using as `string`
   ```lua
   -- WRONG
...
4. **Missing function return types**: Top-level functions need explicit return type annotations
   ```lua
   -- WRONG
...
- [ ] I understand that Rive enforces strict typing by default
