...
- Know why Rive enforces strict typing and why `--!strict` is recommended
...
| Strict mode | N/A | `--!strict` | Recommended; Rive enforces strict typing |
...
This is why Rive enforces strict typing—your animations must work reliably everywhere, and there's no debugger when they run on users' devices.
:::
...
Rive enforces strict type checking by default. The `--!strict` directive is optional, but Rive still expects strict typing and will type-check as if strict is on. Use `--!strict` to make that explicit and keep scripts consistent across tools. This design ensures:
...
When you write a <Term>Node</Term> Script, the Rive compiler expects proper type annotations for your `export type`, function parameters, and return values. Missing or incorrect types will prevent your script from compiling.
...
```lua
--!strict    -- Full type checking (recommended; Rive enforces strict typing)
...
| Used in Rive | **No** | **No** | **Yes (enforced)** |

Even if you omit `--!strict`, Rive still enforces strict typing. The directive is optional but recommended for clarity.

---

## Practice Exercises

### Exercise 1: Parse Text Safely ⭐ {#exercise-1-parse-text-safely}
...
A UI field or designer <Term>Input</Term> provides a string value, but you need numeric math for animation. Parsing explicitly keeps strict mode happy and avoids hidden coercion bugs.
...
### Exercise 2: Optional Label ⭐ {#exercise-2-optional-label}
...
### Exercise 3: Return Paths ⭐ {#exercise-3-return-paths}

### Premise

Strict mode requires every code <Term>Path</Term> to return a value that matches the function signature. Clear boolean helpers avoid hidden nil returns.
...
### Exercise 4: Literal Union Mode ⭐ {#exercise-4-literal-union-mode}

### Premise

Literal unions limit a variable to known values. This keeps <Term>State Machine</Term> values predictable and typo-proof.
...
### Exercise 5: Table Shape Enforcement ⭐⭐ {#exercise-5-table-shape-enforcement}
...
### Exercise 6: Input Safety ⭐⭐ {#exercise-6-input-safety}
...
<Quiz
  ...
  type="code"
  answer="--!strict"
  explanation="The --!strict directive enables explicit strict type checking. In Rive it is optional because strict typing is enforced anyway, but keep it as the first line if you include it."
/>
...
<Quiz
  question="Why does Rive enforce strict typing (and recommend --!strict)?"
  id="strict-q4"
  options={[
  ...
  explanation="Strict typing catches type errors before your animation runs, provides better autocomplete in the editor, and ensures more reliable runtime behavior."
/>
...
1. **Omitting `--!strict`**: Optional, but recommended to keep strict typing explicit (Rive enforces strict typing regardless)
   ```lua
...
- [ ] I understand why `--!strict` is optional but recommended in Rive
...
- Continue to [2.2 Custom Types & Unions](/types/custom-types)
