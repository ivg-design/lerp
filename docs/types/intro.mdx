...
description: Understand strict mode (optional --!strict directive), add type annotations, and understand type inference
keywords:
  - rive
  - luau
  - scripting
...
## Learning Objectives
- Understand strict mode and the optional `--!strict` directive
...
**Luau with `--!strict` catches type errors BEFORE your code runs.** This is why Rive enforces strict typing—bugs are caught during development, not when users see your animation.
:::
...
Rive enforces strict type checking by default. The `--!strict` directive is optional but recommended; strict typing is enforced either way. Your script must define types for persistent state and inputs, and the engine enforces them.
...
Add `--!strict` as the first line to make strict typing explicit. Rive enforces strict typing even if you omit the directive.
:::
...
<Quiz
  ...
  type="code"
  answer="--!strict"
  explanation="Add --!strict as the first line to make strict typing explicit. Rive enforces strict typing even if you omit the directive."
/>
...
1. **Omitting `--!strict`**: Optional, but recommended to keep strict typing explicit
   ```lua
   -- WRONG: No directive
...
- [ ] I prefer to include `--!strict` for clarity (optional; strict typing enforced)
