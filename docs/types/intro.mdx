...
description: Understand strict mode, add type annotations, and understand type inference
keywords:
  - rive
  - luau
  - scripting
...
## Learning Objectives
- Understand strict mode and how Rive enforces strict typing
...
| Type checking mode | None (runtime only) | Strict (enforced by Rive) | Luau catches errors at compile time |
...
**Luau strict mode catches type errors BEFORE your code runs.** This is why Rive enforces strict typing—bugs are caught during development, not when users see your animation.
:::
...
Rive enforces strict type checking by default. Your script must define types for persistent state and inputs, and the engine enforces them.
...
4. **Redundant annotations**: Don't annotate when inference is obvious
   ```lua
