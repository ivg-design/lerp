...
Rive enforces strict type checking by default. The `--!strict` directive is optional but recommended; strict typing is enforced either way. Your script must define types for persistent state and inputs, and the engine enforces them at compile time.
...
**Key elements:**
1. `--!strict` - Explicitly enables strict type checking (optional but recommended)
