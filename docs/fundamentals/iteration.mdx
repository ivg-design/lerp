...
Rive does not move to the next script line until the loop ends. If a loop inside `init()` never ends, setup never finishes. In affected editor builds, that can make the file or editor unresponsive. Keep while loops small, bounded, and easy to exit while learning.
