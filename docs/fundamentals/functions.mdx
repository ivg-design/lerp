...
Closures are especially useful in Rive for callbacks and listeners:

```lua
export type ButtonScript = {
    clickCount: number,
    button: Input<Artboard<Data.ButtonVM>?>,
}
...
function init(self: ButtonScript): boolean
    ...
    if self.button.value then
        -- This closure captures 'self'
        self.button.value:addListener(function(event: ButtonEvent)
            if event.type == "click" then
                self.clickCount += 1  -- Modifies self through closure
