...
### Arrays: Use `ipairs` or Direct Loop
...
-- Method 2: direct iteration (Luau shortcut)
for i, item in items do
    print(`{i}: {item}`)
end

-- Method 3: numeric loop (most control)
for i = 1, #items do
    print(`{i}: {items[i]}`)
end
```
...
### Dictionaries: Use `pairs` or Direct Loop
...
for k, v in tbl do ... end           -- Both (Luau)
