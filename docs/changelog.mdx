...
### Fixed
- **OOP Exercise 6 (Read-Only Table)** — corrected the exercise to use a proxy-table read-only pattern (`source` + `proxy`) so writes to existing keys are properly blocked through `__newindex` on the proxy.
- **Exercise registry sync** — updated `oop-indexmeta-6` hints and explanation to reflect true Luau metamethod behavior.
